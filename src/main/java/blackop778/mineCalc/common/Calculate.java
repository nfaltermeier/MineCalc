...
public class Calculate extends CommandBase {
	...
	if (playerHasMod(getEntityPlayerFromCommandSender(sender)))
	...
	if (sender == null)
	    ...
	    Entity e = getEntityPlayerFromCommandSender(sender);
	    if (e == null)
		return Calculator.consoleLastOutput;
	    IMineCalcCompound comp = MineCalcCompound.get(e);
	...
	if (ent == null)
	    return true;
	return MineCalcCompound.get(ent).getHasMineCalc();
    }
     ...
     */
    private EntityPlayer getEntityPlayerByName(World world, String name) {
	@SuppressWarnings("unchecked")
     ...
     * @return Player or null if not found
     */
    private EntityPlayer getEntityPlayerFromCommandSender(ICommandSender sender) {
	return getEntityPlayerByName(sender.getEntityWorld(), sender.getCommandSenderName());
    }
}